...
const Index: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const description =
      'AI scenarios that challenge your mind. Practice real skills or explore for fun with 200 free credits monthly. No credit card required.';
  ...
  const stats = [
    { label: 'Unique scenarios', value: '1,000+' },
    { label: 'Multi‑character interactions', value: 'Yes' },
    { label: 'New content weekly', value: '✓' },
    { label: 'Active creators', value: 'Community‑driven' },
  ];
  ...
  const categories = [
    {
      ...
      items: [
        { name: 'Business & Leadership', desc: 'Navigate office politics, lead teams through crises' },
        { name: 'Communication Skills', desc: 'Master difficult conversations, negotiations' },
        { name: 'Customer Service', desc: 'Handle challenging situations with grace' },
      ],
    },
    {
      ...
      items: [
        { name: 'Historical Simulations', desc: 'Advise historical figures, change history' },
        { name: 'Science & Ethics', desc: 'Tackle moral dilemmas, explore hypotheticals' },
        { name: 'Language Practice', desc: 'Conversational learning with native AI speakers' },
      ],
    },
    {
      ...
      items: [
        { name: 'Mystery & Detective', desc: 'Solve crimes by interrogating AI suspects' },
        { name: 'Sci‑Fi Adventures', desc: 'Command space stations, first contact scenarios' },
        { name: 'Social Experiments', desc: 'Navigate complex social dynamics' },
      ],
    },
    {
      ...
      items: [
        { name: 'Storytelling', desc: 'Co‑create narratives with AI characters' },
        { name: 'Problem Solving', desc: 'Escape rooms, logic puzzles with AI partners' },
        { name: 'Role Reversal', desc: 'Play the villain, mentor, or wildcard' },
      ],
    },
  ];
      ...
      <main role="main">
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6 py-20 relative">
            <div className="max-w-3xl">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 max-w-xl">
                <Button
                  size="lg"
                  aria-label="Sign up free and get monthly credits"
                  className="bg-cyan-400 text-slate-900 px-6 py-3 rounded-lg font-medium transition-all duration-200 hover:bg-cyan-300 hover:scale-105 shadow-lg shadow-cyan-400/30"
                  onClick={() => navigate('/register')}
                >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mt-10">
                {stats.map((s) => (
                  <div key={s.label} className="bg-slate-800/60 border border-gray-700 rounded-xl p-4 text-center">
        ...
        <section aria-labelledby="categories-title" className="py-16 md:py-20 border-t border-gray-700/60">
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {categories.map((cat) => (
                <div key={cat.title} className={`rounded-xl border border-gray-700 bg-gradient-to-br ${cat.color} p-5`}>
        ...
        <section aria-labelledby="showcase-title" className="py-16 md:py-20 border-t border-gray-700/60">
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {featured.map((s) => (
                <div key={s.title} className="bg-slate-800 border border-gray-700 rounded-xl p-5">
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {s.tags.map((t) => (
                      <Badge key={t} variant="secondary" className="bg-slate-700 text-slate-200 border border-gray-600">
                        {t}
                      </Badge>
                    ))}
                  </div>
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <ul className="space-y-3">
                {[
                  'Not just chat—real scenarios with multiple characters, objectives, and outcomes',
                  'Professional‑grade training scenarios for real skill development',
                  'Engaging stories and adventures that rival interactive fiction games',
                  'Advanced AI that maintains character consistency in long conversations',
                  'Generous free tier — 200 credits monthly means you can always play',
                ].map((text) => (
              ...
              <div className="rounded-xl border border-gray-700 bg-slate-800 p-5">
                ...
                <ul className="space-y-2 text-slate-300 text-sm">
                  {['Secure & Private', 'Your conversations stay yours', 'Transparent pricing', 'No hidden fees', 'Cancel anytime'].map((t) => (
                    <li key={t} className="flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-cyan-400" /> {t}</li>
                  ))}
                </ul>
              </div>
            </div>
        ...
        <section aria-labelledby="pricing-title" className="py-16 md:py-20 border-t border-gray-700/60">
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
              {plans.map((p) => (
                <div key={p.name} className={`rounded-xl border ${p.popular ? 'border-cyan-400 shadow-lg shadow-cyan-400/20' : 'border-gray-700'} bg-slate-800 p-6`}>
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-lg font-semibold text-white">{p.name}</h3>
                    {p.popular && (
                      <span className="text-xs px-2 py-1 rounded bg-cyan-400 text-slate-900 font-medium">Most Popular</span>
                    )}
                  </div>
                  <p className="text-slate-300 mb-4">{p.price}</p>
                  <ul className="space-y-2 text-sm text-slate-300">
                    {p.highlights.map((h) => (
                      <li key={h} className="flex items-start gap-2"><Sparkles className="w-4 h-4 text-cyan-400 mt-0.5" /> {h}</li>
                    ))}
                  </ul>
...
const InfinityIcon: React.FC<{ className?: string }> = ({ className = '' }) => (
  ...
  >
    <path
      d="M18 8c-2 0-3.5 1.5-6 4-2.5-2.5-4-4-6-4C3.79 8 2 9.79 2 12s1.79 4 4 4c2 0 3.5-1.5 6-4 2.5 2.5 4 4 6 4 2.21 0 4-1.79 4-4s-1.79-4-4-4Z"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
