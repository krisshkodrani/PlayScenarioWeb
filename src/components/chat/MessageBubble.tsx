...
const MessageBubble: React.FC<MessageBubbleProps> = ({
    ...
    return <div className="w-full">
        ...
        <div className="flex items-start justify-end sm:w-[90%] md:w-[80%] lg:w-[72%] xl:w-[68%] ml-auto">
          <div className="relative min-w-0">
            ...
            <div className={`absolute -top-2 -right-2 z-10 w-6 h-6 rounded-full flex items-center justify-center shadow-lg ${isActionMessage ? 'bg-amber-500 text-slate-900' : 'bg-cyan-500 text-slate-900'}`}>
              {isActionMessage ? <Zap className="w-3 h-3" /> : <MessageCircle className="w-3 h-3" />}
            </div>
            
            {/* Bubble */}
            <div className={`backdrop-blur px-4 py-3 rounded-2xl rounded-br-sm shadow-lg border break-words whitespace-pre-wrap text-slate-200 border-slate-600 ${isActionMessage ? 'bg-slate-750/40' : 'bg-slate-750'} ${isOptimistic ? 'opacity-70' : ''} min-w-0 max-w-full`}>
    ...
    return <div className="w-full">
        <div className="flex items-start gap-4 sm:w-[90%] md:w-[80%] lg:w-[72%] xl:w-[68%]">
          <div className="flex-1 relative min-w-0">
            <div className="max-w-full bg-slate-800/70 border border-violet-900/30 text-slate-300 px-6 py-4 rounded-xl">
  ...
  return <div className="w-full">
      <div className="flex items-start gap-4 group sm:w-[90%] md:w-[80%] lg:w-[72%] xl:w-[68%]">
        ...
        <div className="flex-1 relative min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1 ml-1">
            ...
            {message.mode && <div className={`w-auto h-5 px-2 rounded-full flex items-center justify-center border text-[10px] tracking-wide uppercase ${isActionResponse ? 'border-amber-500/40 text-amber-400' : 'border-cyan-500/40 text-cyan-400'}`}>
          ...
          {/* Message Content */}
          <div className={`backdrop-blur px-4 py-3 rounded-2xl rounded-bl-sm shadow-lg border ${isActionResponse ? 'bg-slate-750/40 text-slate-200 border-slate-600' : 'bg-slate-750 text-slate-200 border-slate-600'}`}>
          ...
          <div className="absolute -bottom-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            ...
            {onSuggestionClick && ((parsedData?.suggested_follow_ups || message.suggested_follow_ups)?.length ?? 0) > 0 && <button onClick={() => setShowSuggestionsModal(true)} className="bg-slate-700/80 backdrop-blur border border-slate-600 text-slate-400 hover:text-cyan-400 hover:border-cyan-400/50 p-1.5 rounded-lg transition-all duration-200" aria-label="Show follow-up suggestions">
      ...
      <SuggestionsModal isOpen={showSuggestionsModal} onClose={() => setShowSuggestionsModal(false)} suggestions={(parsedData?.suggested_follow_ups || message.suggested_follow_ups) ?? []} onSelect={s => {
      onSuggestionClick?.(s);
      setShowSuggestionsModal(false);
    }} />

      <FeedbackModal isOpen={showFeedbackModal} onClose={() => setShowFeedbackModal(false)} onSubmit={handleFeedbackSubmit} feedbackType={feedbackType} isSubmitting={isSubmittingFeedback} />
    </div>;
};
