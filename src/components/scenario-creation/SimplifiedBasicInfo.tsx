      ...
      <Card className="bg-slate-800 border-slate-700">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea id="initial-scene" value={data.initial_scene_prompt} onChange={e => onChange({
            initial_scene_prompt: e.target.value
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <Switch id="show-difficulty" checked={data.show_difficulty ?? true} onCheckedChange={checked => onChange({
              show_difficulty: checked
            })} />
              <Label htmlFor="show-difficulty" className="text-sm text-white">Enable</Label>
            </div>
          </div>

          {(data.show_difficulty ?? true) && <div className="space-y-2">
              <Label className="text-white">Difficulty Level</Label>
              <Select value={data.difficulty || 'beginner'} onValueChange={value => onChange({
            difficulty: value as 'beginner' | 'intermediate' | 'advanced' | 'expert'
                ...
                <SelectContent>
                  {difficultyLevels.map(level => <SelectItem key={level.value} value={level.value}>
                      <div>
      ...
      <Card className="bg-slate-800 border-slate-700">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            ...
            <Slider value={[data.max_turns]} onValueChange={([value]) => onChange({
            max_turns: value
          })} max={50} min={5} step={5} className="w-full" />
