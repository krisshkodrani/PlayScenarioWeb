...
const SimplifiedCharacters: React.FC<SimplifiedCharactersProps> = ({
  ...
  return <div className="space-y-6">
        ...
        <CardContent className="space-y-6">
          ...
          <Tabs defaultValue="create" className="w-full">
            ...
            <TabsContent value="create" className="space-y-4 mt-6">
              {!showForm && <div className="text-center py-6 border-2 border-dashed border-slate-600 rounded-lg">
                  ...
                  <Button onClick={() => setShowForm(true)} className="bg-cyan-500 hover:bg-cyan-600 text-white">
                    <Plus className="w-4 h-4 mr-2" />
                    Create Character
                  </Button>
                </div>}
              ...
              {showForm && <Card className="bg-slate-700/50 border-slate-600">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input id="char-name" value={formData.name} onChange={e => setFormData(prev => ({
                    ...prev,
                    name: e.target.value
                  }))} placeholder="Enter character name" className="bg-slate-600 border-slate-500 text-white placeholder-slate-400 focus:border-cyan-400" maxLength={50} />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="char-personality" className="text-white">Personality *</Label>
                      <Textarea id="char-personality" value={formData.personality} onChange={e => setFormData(prev => ({
                    ...prev,
                    personality: e.target.value
                  }))} placeholder="Describe the character's personality, behavior, and speaking style..." rows={4} className="bg-slate-600 border-slate-500 text-white placeholder-slate-400 focus:border-cyan-400 resize-none" maxLength={500} />
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        <Input value={newKeyword} onChange={e => setNewKeyword(e.target.value)} placeholder="Add expertise..." className="bg-slate-600 border-slate-500 text-white placeholder-slate-400 focus:border-cyan-400" onKeyPress={e => e.key === 'Enter' && addKeyword()} />
                        <Button onClick={addKeyword} disabled={!newKeyword.trim()} variant="outline" className="border-slate-500 text-slate-300 hover:text-white">
                          <Plus className="w-4 h-4" />
                        </Button>
                      </div>
                      
                      {formData.expertise_keywords.length > 0 && <div className="flex flex-wrap gap-2">
                          {formData.expertise_keywords.map((keyword, index) => <Badge key={index} variant="secondary" className="flex items-center gap-1 bg-slate-600 text-slate-200">
                              {keyword}
                              <Button variant="ghost" size="sm" className="h-auto p-0 ml-1 hover:text-red-400" onClick={() => removeKeyword(index)}>
                                <X className="w-3 h-3" />
                              </Button>
                            </Badge>)}
                        </div>}
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox id="is-player" checked={formData.is_player_character} onCheckedChange={checked => setFormData(prev => ({
                    ...prev,
                    ...
                    <div className="flex gap-3 pt-4">
                      <Button onClick={saveCharacter} disabled={!formData.name.trim() || !formData.personality.trim()} className="bg-cyan-500 hover:bg-cyan-600 text-white">
                        {editingIndex !== null ? 'Update Character' : 'Add Character'}
                      </Button>
                      <Button onClick={resetForm} variant="outline" className="border-slate-500 text-slate-300 hover:text-white">
                        Cancel
                      </Button>
                    </div>
                  </CardContent>
            ...
            <TabsContent value="browse" className="space-y-4 mt-6">
              <div className="text-center py-6 border-2 border-dashed border-slate-600 rounded-lg">
                ...
                <Button onClick={() => setShowCharacterBrowser(true)} className="bg-violet-500 hover:bg-violet-600 text-white">
                  <Library className="w-4 h-4 mr-2" />
          ...
          {data.characters.length > 0 && <div className="space-y-4">
              <div className="flex items-center justify-between">
              ...
              <div className="grid gap-4">
                {data.characters.map((character, index) => <Card key={index} className="bg-slate-700/30 border-slate-600">
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between">
                        <div className="flex-1 space-y-2">
                          ...
                          {character.expertise_keywords.length > 0 && <div className="flex flex-wrap gap-1">
                              {character.expertise_keywords.slice(0, 3).map((keyword, idx) => <Badge key={idx} variant="outline" className="text-xs text-slate-400 border-slate-500">
                                  {keyword}
                                </Badge>)}
                              {character.expertise_keywords.length > 3 && <Badge variant="outline" className="text-xs text-slate-500 border-slate-600">
                                  +{character.expertise_keywords.length - 3}
                                </Badge>}
                            </div>}
                        </div>
                        <div className="flex gap-2 ml-4">
                          <Button variant="ghost" size="sm" onClick={() => editCharacter(index)} className="text-slate-400 hover:text-white h-8 w-8 p-0">
                            <Edit className="w-4 h-4" />
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => removeCharacter(index)} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 h-8 w-8 p-0">
          ...
          {data.characters.length === 0 && <div className="text-center py-8">
      ...
      <CharacterBrowserModal isOpen={showCharacterBrowser} onClose={() => setShowCharacterBrowser(false)} onSelectCharacters={handleAddFromLibrary} excludeCharacterIds={[]} // We'll use name matching instead for simplicity
    />
    </div>;
};
