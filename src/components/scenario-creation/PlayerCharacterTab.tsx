...
const PlayerCharacterTab: React.FC<PlayerCharacterTabProps> = ({ data, onChange }) => {
  ...
  const handleSelectPlayerCharacter = (selectedCharacters: CharacterData[]) => {
    if (selectedCharacters.length > 0) {
      const newPlayerCharacter = { ...selectedCharacters[0], is_player_character: true };
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex items-start space-x-3 p-4 rounded-lg border border-slate-600 hover:border-slate-500 transition-colors">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-slate-400">
                  Play without a specific character identity. You'll interact as yourself in the scenario.
                </p>
              </div>
            </div>
                      ...
                      <div className="bg-slate-700/50 rounded-lg p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            {playerCharacter.expertise_keywords.length > 0 && (
                              <div className="flex flex-wrap gap-1">
                                {playerCharacter.expertise_keywords.slice(0, 3).map((keyword, index) => (
                                  <span key={index} className="text-xs bg-cyan-500/20 text-cyan-300 px-2 py-1 rounded">
                                    {keyword}
                                  </span>
                                ))}
                                {playerCharacter.expertise_keywords.length > 3 && (
                        ...
                        <div className="flex gap-2 mt-3">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={handleRemovePlayerCharacter}
      ...
      <CharacterBrowserModal
        ...
        onSelectCharacters={handleSelectPlayerCharacter}
        excludeCharacterIds={data.characters.filter(char => !char.is_player_character).map(char => char.id).filter(Boolean)}
        maxSelection={1}
        title="Choose Player Character"
        description="Select a character from your library to play as in this scenario"
      />
    </div>
  );
};
